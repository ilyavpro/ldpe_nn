...
# Все колонки (укажи корректный список для своего датасета)
all_cols = [
...
# Новые выходные переменные
y_cols = [
    'MWN_LDPE','MWW_LDPE','FSCBN_LDPE','TMAX_PFR1','TEMP_OUT','FLOW_LDPE'
]
# Новые входные переменные — все остальные
X_cols = [col for col in all_cols if col not in y_cols]

X = data[X_cols].values
y = data[y_cols].values

# 2. Масштабирование (обрати внимание: fit только на train - если захочешь избежать data leakage)
...
class LDPE_Net(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(len(X_cols), 64),  # обновлено
            nn.ReLU(),
            nn.BatchNorm1d(64),
            nn.Dropout(0.5),
            nn.Linear(64, 64),
            ...
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.Linear(32, len(y_cols))   # обновлено
        )
    def forward(self, x):
        return self.net(x)
