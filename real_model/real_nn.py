...
data = pd.read_csv(rf'C:\Users\Ilya\Desktop\AES\code\ldpe_nn\real_model\dataset\merged_tables.csv')

drop_cols = ['Итерация', 'NGRADE', 'LDPE', 'MWW', 'MWN', 'FSCB']
y_cols = ['LDPE', 'MWW', 'MWN', 'FSCB']

# One-hot NGRADE
data = pd.get_dummies(data, columns=['NGRADE'])
X_cols = [col for col in data.columns if col not in drop_cols]
...
# 4. Модель (размер входа и выхода под датасет)
class LDPE_Net(nn.Module):
    def __init__(self, input_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm1d(32),
            nn.Linear(32, 4)  # 4 выхода
        )
    def forward(self, x):
        return self.net(x)

model = LDPE_Net(input_dim=X.shape[1])
...
for i, col in enumerate(y_cols):
    ...
    print(f"{col:>8}: RMSE={rmse:.4f}, MAE={mae:.4f}, MSE={mse:.4f}, R2={r2:.4f}")
