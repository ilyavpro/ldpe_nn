...
N_SAMPLES = 500
VARIATION = 0.2  # Только для выбранных input_params
batch_size = 50

# --- Fixed-параметры с индивидуальными диапазонами
fixed_input_params = [
    {"name": "E2FD_TEMP",    "path": r"\Data\Streams\E2FD\Input\TEMP\MIXED",        "min": 95,    "max": 105},
    {"name": "E2FD_FLOW",    "path": r"\Data\Streams\E2FD\Input\FLOW\MIXED\E2",     "min": 60000, "max": 70000},
    {"name": "PFR1_TEMP",    "path": r"\Data\Blocks\PFR1\Input\TEMP",               "min": 160,   "max": 180},
    {"name": "INIFD1_FLOW1", "path": r"\Data\Streams\INIFD1\Input\FLOW\MIXED\INI1", "min": 4,     "max": 8},
    {"name": "INIFD1_FLOW2", "path": r"\Data\Streams\INIFD1\Input\FLOW\MIXED\INI2", "min": 1,     "max": 2},
]

# --- Варьируемые (и фиксированные) реакционные параметры
input_params = [
...
# --- Только эти input_params варьируются:
vary_input_names = ["FRPRE_EXP_0"]
...
file_name = fr"{base_path}\datasets\partly_{datetime.now().strftime('%m-%d_%H-%M')}.csv"

# --- Порядок колонок: сначала input_params, затем fixed, затем выходы
column_names = (
...
for i in tqdm(range(N_SAMPLES)):
    ...
    # --- Варьируем только нужные input_params, остальные фиксированы
    for param in input_params:
        if param["name"] in vary_input_names:
    ...
    # --- Все fixed_input_params варьируются в заданных диапазонах
